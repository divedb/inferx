...
cudaError_t LaunchTyped(const void* query, const void* key, const int32_t* positions,
                        ...
                        uint64_t query_heads, uint64_t kv_heads, float theta,
                        cudaStream_t stream) {
  ...
  const uint32_t blocks =
      static_cast<uint32_t>((tokens + bdy - 1) / bdy) > 0
  ...
  flashinfer::BatchQKApplyRotaryPosIdsKernel</*interleave=*/false, kHeadDim, kVecSize, bdx, T,
      ...
      <<<grid, block, 0, stream>>>(
          ...
          static_cast<T*>(query_output), static_cast<T*>(key_output),
          const_cast<int32_t*>(positions), static_cast<uint32_t>(tokens),
          static_cast<uint32_t>(query_heads), static_cast<uint32_t>(kv_heads),
          /*rotary_dim=*/kHeadDim,
...
cudaError_t LaunchForHeadDim(const void* query, const void* key, const int32_t* positions,
  ...
  return LaunchTyped<T, kHeadDim, kVecSize>(query, key, positions, query_output, key_output,
                                            tokens, query_heads, kv_heads, theta, stream);
}
...
cudaError_t LaunchFlashInferRope(const void* query, const void* key, const int32_t* positions,
  ...
  switch (dtype) {
    ...
    case StorageType::kBFloat16:
      return DispatchHeadDim<__nv_bfloat16>(query, key, positions, query_output, key_output,
                                            tokens, query_heads, kv_heads, head_dimension, theta,
                                            stream);
    default:
      return cudaErrorInvalidValue;
  }
