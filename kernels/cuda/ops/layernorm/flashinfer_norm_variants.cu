...
cudaError_t LaunchFusedAddTyped(const void* input, const void* residual, const void* weight,
  ...
  flashinfer::norm::FusedAddRMSNormKernel<kVecSize, T>
      <<<dim3(static_cast<uint32_t>(tokens)), dim3(kWarpSize, num_warps), shared_bytes, stream>>>(
          static_cast<T*>(const_cast<void*>(input)), static_cast<T*>(const_cast<void*>(residual)),
...
template <typename T, uint32_t kVecSize>
cudaError_t LaunchGemmaTyped(const void* input, const void* weight, void* output, uint64_t tokens,
...
template <typename T, uint32_t kVecSize>
cudaError_t LaunchQkTyped(void* query, void* key, const void* query_weight, const void* key_weight,
                          uint64_t tokens, uint64_t query_heads, uint64_t kv_heads,
                          uint64_t head_dim, float epsilon, cudaStream_t stream) {
  ...
  const uint32_t q_blocks = static_cast<uint32_t>((q_jobs + kWarps - 1) / kWarps) > 0
                                ? static_cast<uint32_t>((q_jobs + kWarps - 1) / kWarps)
                                : 1;
  const uint32_t k_blocks = static_cast<uint32_t>((k_jobs + kWarps - 1) / kWarps) > 0
  ...
  flashinfer::norm::QKRMSNormKernel<kVecSize, T><<<q_blocks, dim3(kWarpSize, kWarps), 0, stream>>>(
  ...
  flashinfer::norm::QKRMSNormKernel<kVecSize, T><<<k_blocks, dim3(kWarpSize, kWarps), 0, stream>>>(
      static_cast<T*>(key), static_cast<T*>(const_cast<void*>(key_weight)), static_cast<T*>(key),
      head, static_cast<uint32_t>(tokens), static_cast<uint32_t>(kv_heads), k_stride_n, head,
      k_stride_n, head, 0.0F, epsilon);
  return cudaGetLastError();
}
...
cudaError_t LaunchFlashInferFusedAddRmsNorm(const void* input, const void* residual,
  ...
  switch (dtype) {
    ...
    case StorageType::kBFloat16:
      return LaunchFusedAddTyped<__nv_bfloat16, 8>(input, residual, weight, tokens, hidden, epsilon,
                                                   stream);
    default:
