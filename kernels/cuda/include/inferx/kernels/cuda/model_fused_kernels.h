...
cudaError_t LaunchMhcPre(const void* residual, const void* fn, const void* hc_scale,
                         const void* hc_base, void* mix_workspace, void* layer_input,
                         void* post, void* comb, uint64_t tokens, uint32_t streams,
                         uint32_t hidden, float rms_eps, float hc_eps, uint32_t sinkhorn_iters,
                         StorageType dtype, cudaStream_t stream);
...
cudaError_t LaunchRingSconv(const void* x, const void* weight, void* conv_cache,
                            const int32_t* seq_prefix, const int32_t* cache_indices,
                            void* output, uint64_t tokens, uint64_t width, uint32_t window,
                            uint64_t ring, uint64_t batch, StorageType dtype,
                            cudaStream_t stream);
