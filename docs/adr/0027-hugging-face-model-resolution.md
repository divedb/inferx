# ADR 0027: Hugging Face model resolution and cache handoff
...
while ADR 0021's loader correctly rejects every descendant symlink. Passing a standard snapshot
directly to the loader would silently violate one of those contracts.

## Decision

InferX owns a model resolver before the ADR 0021 trust boundary. `MODEL` is an existing local
...
hard links or copies. Only this regular-file view crosses into `ModelLocator`; ADR 0021 remains
...
- Passing standard cache symlinks to `ModelLocator` was rejected because it weakens ADR 0021.
...
the ADR 0021 regular-file handoff.
