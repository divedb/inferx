// Base/startup benchmark smoke: enough calls to prove
...
void BenchmarkGetVersion(benchmark::State& state) {
...
BENCHMARK(BenchmarkGetVersion);

void BenchmarkGetVersionString(benchmark::State& state) {
  for (auto _ : state) {  // NOLINT(clang-analyzer-deadcode.DeadStores)
    benchmark::DoNotOptimize(inferx::GetVersionString());
  }
}
BENCHMARK(BenchmarkGetVersionString);
