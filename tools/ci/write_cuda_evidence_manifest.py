#!/usr/bin/env python3
"""Write the canonical manifest for retained CUDA qualification artifacts."""
...
def main() -> int:
    ...
    manifest = {
        ...
        "outcomes": {
            "tests": os.environ.get("INFERX_CUDA_TEST_OUTCOME", "unknown"),
            "compute_sanitizer": os.environ.get(
                "INFERX_CUDA_SANITIZER_OUTCOME", "unknown"
            ),
            "benchmarks": os.environ.get(
                "INFERX_CUDA_BENCHMARK_OUTCOME", "unknown"
            ),
        },
        "commands": [
            "ctest --test-dir out/build/cuda-release -L <platform-label> --output-on-failure",
            "tools/ci/run_compute_sanitizer.sh --preset cuda-release --suite platform --output out/sanitizer/cuda",
