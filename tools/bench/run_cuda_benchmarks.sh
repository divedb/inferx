...
case "$output" in
  "$source_root"/out/*) ;;
  *) echo "CUDA benchmark output must be under $source_root/out" >&2; exit 2 ;;
esac
mkdir -p "$output"
...
if [[ ! -x "$device_info" ]]; then
  echo "missing CUDA device diagnostic: $device_info" >&2
  exit 1
fi
...
for benchmark in inferx_memory_pool_benchmark inferx_cuda_copy_benchmark \
  ...
  if [[ ! -x "$binary" ]]; then
    echo "missing CUDA benchmark: $binary" >&2
    exit 1
  fi
...
python3 tools/bench/check_cuda_overhead.py \
