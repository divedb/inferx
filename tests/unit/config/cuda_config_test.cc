...
TEST(CudaConfigTest, NestedSectionParsesAndCanonicalizesAsSchemaV2) {
...
TEST(CudaConfigTest, SchemaV1DefaultsRemainByteStable) {
  const EngineConfig config =
...
TEST(CudaConfigTest, SchemaV1OverridesDoNotActivateCudaCrossFieldRules) {
  ParsedConfig parsed;
...
TEST(CudaConfigTest, CapabilityAndCrossFieldFailuresAreRejected) {
...
TEST(CudaConfigTest, JsonTypesRemainStrict) {
