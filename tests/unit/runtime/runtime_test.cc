...
TEST(RuntimeErrorReasonTest, CudaAndLifetimeReasonNamesRoundTrip) {
...
TEST(MemoryTrackerTest, ReservationCommitRollbackPeakAndLimit) {
...
TEST(MemoryTrackerTest, LeakedChargeRemainsDiagnostic) {
  constexpr std::array<MemoryLimit, 1> kLimits{
      MemoryLimit{Device::Host(), MemoryKind::kHost, ByteCount(100)}};
  MemoryTracker tracker = MemoryTracker::Create(kLimits).value();
  const MemoryKey key{Device::Host(), MemoryKind::kHost, MemoryCategory::kTest};
...
TEST(MemoryTrackerTest, ConcurrentSnapshotsRemainWithinDomainLimit) {
  constexpr std::array<MemoryLimit, 1> kLimits{
...
TEST(MemoryTrackerTest, CpuAllocatorUsesSuppliedTransactionalAccounting) {
...
TEST(FakeAllocatorTest, DeviceMetadataAndFailureHooksAreDeterministic) {
...
TEST(BufferPoolTest, LowestSlotExhaustionGenerationAndClose) {
...
TEST(BufferPoolTest, MaximumGenerationRetiresSlot) {
  CpuAllocator allocator;
...
TEST(LifetimeDeathTest, TrackerDestructionRejectsLiveCharges) {
  EXPECT_DEATH(([] {
...
TEST(LifetimeDeathTest, PoolDestructionRejectsLiveLeases) {
  EXPECT_DEATH(([] {
                 CpuAllocator allocator;
                 Buffer backing =
...
TEST(WorkspaceTest, CheckedBumpAllocationDoesNotAdvanceOnFailure) {
  CpuAllocator allocator;
...
TEST(CompletionFenceTest, PendingCompleteAcknowledgeAndStale) {
...
TEST(CompletionFenceTest, FailureAndAbandonAreDeferred) {
  testing::FakeFenceDomain domain(1);
...
TEST(CompletionFenceTest, PendingWaitTimesOutWithoutRecycling) {
  testing::FakeFenceDomain domain(1);
  CompletionFence fence = domain.Acquire().value();
