// Status convention and macro behavior tests (m1.md sections 7.1, 17.1).
...
TEST(MessageConventionTest, ComponentFieldPrefix) {
  ...
  const absl::Status component =
      ComponentError(absl::StatusCode::kUnimplemented, "tokenizer", "text input requires M3");
  EXPECT_EQ(component.message(), "tokenizer: text input requires M3");
}
