...
TEST(OperatorReferenceTest, EmbeddingValidatesEveryIdBeforeWriting) {
...
TEST(OperatorReferenceTest, GemmUsesOutInWeightLayout) {
...
TEST(OperatorReferenceTest, RmsNormAndSwiGluMatchHandValues) {
...
TEST(OperatorReferenceTest, RopeUsesHalfSplitPairing) {
...
TEST(OperatorReferenceTest, AttentionAppendsKvAndMapsGqaHeads) {
...
TEST(OperatorRegistryTest, SelectionIsIndependentOfRegistrationOrder) {
  KernelKey key;
  key.op = OpKind::kGemm;
  key.rank = 3;
  key.dimensions[0] = 1;
...
TEST(OperatorRegistryTest, RequiredReferenceSetWarmsTransactionally) {
...
TEST(OperatorContractTest, ZeroTokenOperationsValidateAsNoOps) {
...
TEST(OperatorContractTest, RejectsPartialAndReadOnlyOperandAliases) {
...
TEST(OperatorReferenceTest, ResourceLimitsRejectBeforeMutation) {
  constexpr std::array<uint64_t, 2> kShape{2, 2};
...
TEST(OperatorReferenceTest, RandomizedGemmMatchesFp64Traversal) {
  std::mt19937 generator(1295274565U);
...
TEST(OperatorRegistryTest, RejectsMalformedKeysAndBoundsPreparedCache) {
...
TEST(OperatorRegistryTest, WarmupFailureDoesNotPublishPartialCache) {
  KernelKey first;
