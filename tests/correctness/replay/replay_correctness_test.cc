...
absl::StatusOr<config::EngineConfig> ReadConfig(const std::string& filename) {
  auto fields = config::ReadConfigFile(std::string(INFERX_REPLAY_DATA_DIR) + "/" + filename);
...
TEST(ReplayCorrectnessTest, ReviewedGoldenTraceMatchesSerializerExactly) {
  ...
  auto workload = ReadWorkloadFile(std::string(INFERX_REPLAY_DATA_DIR) + "/basic_workload.jsonl");
  ASSERT_TRUE(workload.ok()) << workload.status();
  auto golden = ReadReplayInputs(std::string(INFERX_REPLAY_DATA_DIR) + "/basic_trace.jsonl");
